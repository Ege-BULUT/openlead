...
how long it took, how many tokens it burned — is visible in one place instead of scattered
across chat transcripts.

## Why

Agents are good at doing the next task. They're bad at remembering the last ten, at knowing
which one is blocked on which, and at leaving a trail a human (or a different agent, next
week) can actually follow. OpenLead gives them — and you — a durable, boring, local place for
that: a roadmap that says what's next and why, a Kanban board with a real audit trail, and a
journal system so an agent's working notes outlive its context window.
...
- **`roadmap.html`** — milestones with a diagram (auto-laid-out from however many you have,
  colored by status: done / up next / planned) and a detail card per milestone underneath.
...
later re-copies the HTML/scripts (for an upgrade) without ever touching your existing
`data/*.json`.
...
Claude Code agent will pick it up automatically when the conversation is about setting up or
...
`.claude/skills/<name>/SKILL.md` directly, alongside its own `.opencode/skills/` path — same
frontmatter spec (`name` + `description`), so this repo's `SKILL.md` works for both tools
...
HTML (via a targeted regex substitution) — never touching the page's CSS/layout/JS. That's
what lets you freely customize a page's look without a future `render()` call clobbering it.
