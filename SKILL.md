---
name: openlead
description: Scaffold and operate a fully local, portable project-management workspace for AI coding agents, with a homepage, a milestone roadmap (with an auto-generated diagram), a Kanban task board, and per-agent memory journals, all driven by JSON and edited only through the bundled Python CLIs. Use when the user asks to set up a project roadmap, task board, or kanban for a project; when they want agents to track their own work with timestamps/token/duration logs; when they mention wanting a "team lead" view over agent work; or when working inside an existing OpenLead workspace (a directory containing index.html, roadmap.html, tasks.html, memory.html and a data/ + scripts/ pair).
license: MIT
...
timestamped entry to the task's own `log[]`, recording who changed what and when, with zero extra
...
adds a timestamped section to an existing journal (the normal case, since journals are logs, not
